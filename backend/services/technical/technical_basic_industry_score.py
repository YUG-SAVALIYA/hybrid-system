"""
TechnicalBasicIndustryScoreService

Normalizes median relative returns into percentile return scores (comparing only siblings
within the same parent sector and parent industry), aggregates available component scores using configured weights,
...
class TechnicalBasicIndustryScoreService:
    ...
    def calculate_basic_industry_scores(
        ...
        records = self._disc.execute(
            text("""
                SELECT 
                    id, entity_name, parent_sector, parent_industry, 
                    technical_breadth_score, technical_volume_score, technical_consistency_score,
        ...
        # Group by (parent_sector, parent_industry)
        sibling_groups = {}
        for r in records:
            p_sec = (r.parent_sector or "").strip()
            p_ind = (r.parent_industry or "").strip()
            if not p_sec or not p_ind:
                continue
        ...
        values_to_update = []
        for (p_sec, p_ind), group_records in sibling_groups.items():
            ...
            for pr in group_records:
                ...
                if med_ret is not None:
                    if num_valid_siblings == 1:
                        ...
                        ret_rank = 1.0
                        percentile = 50.0
                        if "SINGLE_BASIC_INDUSTRY_COMPARISON" not in pr["warnings"]:
                            pr["warnings"].append("SINGLE_BASIC_INDUSTRY_COMPARISON")
                    else:
