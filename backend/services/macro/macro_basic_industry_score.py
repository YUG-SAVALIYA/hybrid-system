...
class MacroBasicIndustryScoreService:
    ...
    def calculate_basic_industry_scores(self, run_id: str) -> Dict[str, Any]:
        impacts = (
            self._disc.query(MacroEntityImpact)
            .filter_by(run_id=run_id, entity_type=ENTITY_TYPE_BASIC_INDUSTRY)
            .order_by(
    ...
    def _cleanup_stale_scores(self, run_id: str, current_keys: set[Tuple[str, str, str]]) -> int:
        groups = (
            self._disc.query(GroupScore)
            .filter_by(run_id=run_id, entity_type=ENTITY_TYPE_BASIC_INDUSTRY)
            .all()
