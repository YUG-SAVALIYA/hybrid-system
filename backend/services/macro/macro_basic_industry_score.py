"""Deterministic numeric macro scoring for basic-industry group scores."""
...
MIN_BASIC_INDUSTRY_MACRO_COVERAGE = float(
    getattr(config, "MIN_BASIC_INDUSTRY_MACRO_COVERAGE", 75.0)
)

W_PARTIAL = "BASIC_INDUSTRY_MACRO_SCORE_PARTIAL"
W_LOW_COVERAGE = "BASIC_INDUSTRY_MACRO_LOW_COVERAGE"
W_UNAVAILABLE = "BASIC_INDUSTRY_MACRO_SCORE_UNAVAILABLE"
...
W_OVERALL_CONFLICT = "OVERALL_BASIC_INDUSTRY_IMPACT_SCORE_CONFLICT"
W_STALE = "BASIC_INDUSTRY_MACRO_IMPACT_STALE"
...
def calculate_basic_industry_macro_score(
...
class MacroBasicIndustryScoreService:
    def __init__(self, discovery_session: Session):
        self._disc = discovery_session

    def calculate_basic_industry_scores(self, run_id: str, horizon: str | None = None) -> Dict[str, Any]:
        impact_query = self._disc.query(MacroEntityImpact).filter_by(
            run_id=run_id, entity_type=ENTITY_TYPE_BASIC_INDUSTRY
        )
        ...
        metadata = {
            ...
            "unavailable_basic_industry_count": 0,
        ...
        current_keys = {
            (
                impact.parent_sector or "",
                impact.parent_industry or "",
                impact.entity_name or "",
            )
        ...
        for impact in impacts:
            group_query = self._disc.query(GroupScore).filter_by(
                ...
                parent_industry=impact.parent_industry,
            )
            if horizon is not None:
                group_query = group_query.filter_by(horizon=impact.horizon)
            groups = group_query.all()
            details, warnings = calculate_basic_industry_macro_score(
                impact.category_impacts or {},
                impact.overall_impact or {},
            )
            details["source_impact_id"] = impact.id
            details["parent_sector"] = impact.parent_sector or ""
            details["parent_industry"] = impact.parent_industry or ""
        ...
        metadata["stale_score_count"] += self._cleanup_stale_scores(
            ...
            horizon,
            {
                (impact.parent_sector or "", impact.parent_industry or "", impact.entity_name or "")
                for impact in impacts
            },
        )
        self._disc.commit()
    ...
    def _apply_score(self, group: GroupScore, details: Dict[str, Any], warnings: List[str]) -> None:
        ...
        macro["basic_industry_score"] = copy.deepcopy(details)
        calc["macro"] = macro
    ...
    def _cleanup_stale_scores(self, run_id: str, horizon: str | None, current_keys: set[Tuple[str, str, str]]) -> int:
        group_query = self._disc.query(GroupScore).filter_by(
            run_id=run_id,
            entity_type=ENTITY_TYPE_BASIC_INDUSTRY,
        )
        ...
        for group in groups:
            calc = copy.deepcopy(group.calculation_details or {})
            macro = copy.deepcopy(calc.get("macro") or {})
            if "basic_industry_score" not in macro:
                continue
            key = (group.parent_sector or "", group.parent_industry or "", group.entity_name or "")
            if key in current_keys:
                continue
            macro["basic_industry_score"] = {
                "available": False,
    ...
    def _count_unique_result(
        ...
        if status == "N_A":
            metadata["n_a_basic_industry_count"] += 1
        elif status == "UNAVAILABLE":
            metadata["unavailable_basic_industry_count"] += 1
        else:
