"""
FundamentalBasicIndustryScoreService

Calculates the final deterministic basic-industry fundamental score based on pillar scores.
"""
...
class FundamentalBasicIndustryScoreService:
    def __init__(self, discovery_session: Session):
        self._disc = discovery_session

    def calculate_basic_industry_scores(self, run_id: str) -> None:
        groups = self._disc.query(GroupScore).filter_by(
            run_id=run_id, 
        ...
        for grp in groups:
            calc = copy.deepcopy(grp.calculation_details or {})
            ...
            warnings_set = set(grp.warnings or [])
            warnings_set.discard("BASIC_INDUSTRY_FUNDAMENTAL_SCORE_PARTIAL")
            warnings_set.discard("BASIC_INDUSTRY_FUNDAMENTAL_LOW_COVERAGE")
            warnings_set.discard("BASIC_INDUSTRY_FUNDAMENTAL_SCORE_UNAVAILABLE")
            ...
            eligible = False
            if final_score is not None and cov_pct >= MIN_GROUP_FUNDAMENTAL_COVERAGE and avail_cnt >= MIN_BASIC_INDUSTRY_FUNDAMENTAL_COMPANIES:
                eligible = True
                
            if available_w == 0:
                warnings_set.add("BASIC_INDUSTRY_FUNDAMENTAL_SCORE_UNAVAILABLE")
            elif available_w < applicable_w:
                warnings_set.add("BASIC_INDUSTRY_FUNDAMENTAL_SCORE_PARTIAL")
                
            if cov_pct < MIN_GROUP_FUNDAMENTAL_COVERAGE:
                warnings_set.add("BASIC_INDUSTRY_FUNDAMENTAL_LOW_COVERAGE")
                
            if avail_cnt < MIN_BASIC_INDUSTRY_FUNDAMENTAL_COMPANIES:
            ...
            calc["fundamental"]["final_score"] = {
                ...
                "minimum_constituents_required": MIN_BASIC_INDUSTRY_FUNDAMENTAL_COMPANIES,
                "eligible_for_selection": eligible
            }
            
            grp.fundamental_score = final_score
            
            w_list = list(warnings_set)
            w_list.sort()
            grp.warnings = w_list
            grp.calculation_details = calc
