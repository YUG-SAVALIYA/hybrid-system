...
Calculates percentile normalization of raw industry fundamental medians within parent sectors.
"""
...
class FundamentalIndustryMetricNormalizationService:
    def __init__(self, discovery_session: Session):
        self._disc = discovery_session

    def normalize_industry_metrics(self, run_id: str) -> None:
        ...
        for ind in industries:
            if ind.parent_sector:
                parent_sectors.setdefault(ind.parent_sector, []).append(ind)
                
        for parent_sector, siblings in parent_sectors.items():
            metrics_to_score = {}
            for metric in HIGHER_IS_BETTER | LOWER_IS_BETTER:
                metrics_to_score[metric] = {}
                
            for ind in siblings:
                calc = ind.calculation_details or {}
                ...
                for m_name in HIGHER_IS_BETTER | LOWER_IS_BETTER:
                    ...
                    if m_name in ["debt_to_equity", "borrowing_change_pct"] and reason == "N_A_NO_STANDARD_DEBT_RULE_COMPANIES":
                        ...
                        if avail_count < MIN_INDUSTRY_FUNDAMENTAL_COMPANIES:
                    ...
                    metrics_to_score[m_name][ind.id] = {
                        "val": val,
            ...
            scores_map = {}
            for m_name, ind_data in metrics_to_score.items():
                direction = "HIGHER_IS_BETTER" if m_name in HIGHER_IS_BETTER else "LOWER_IS_BETTER"
                
                eligible_vals = {ind_id: d["val"] for ind_id, d in ind_data.items() if d["eligible"]}
                scores = _score_percentiles(eligible_vals, direction)
                
                for ind_id, d in ind_data.items():
                    scores_map.setdefault(ind_id, {})[m_name] = {
                        "raw_median": d["val"],
                        "direction": direction,
                    ...
                    if d["eligible"]:
                        if len(eligible_vals) == 1:
                            scores_map[ind_id][m_name]["score"] = 50.0
                            scores_map[ind_id][m_name]["reason"] = "SINGLE_INDUSTRY_METRIC_COMPARISON"
                        else:
                            scores_map[ind_id][m_name]["score"] = scores[ind_id]
                            
            for ind in siblings:
                ind_scores = scores_map.get(ind.id, {})
                
                app_cnt = 0
                scored_cnt = 0
                for m_name, s_data in ind_scores.items():
