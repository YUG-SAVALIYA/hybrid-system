"""
FundamentalSectorPillarScoreService

Calculates the four deterministic sector fundamental pillar scores.
"""
...
class FundamentalSectorPillarScoreService:
    ...
    def calculate_pillar_scores(
        ...
        for sector in sectors:
            ...
            # Remove existing warnings we might regenerate
            ...
            dte_metric = norm_metrics.get("debt_to_equity", {})
            fs_applicable = True
            fs_reason = None
            if not dte_metric.get("applicable", True):
                fs_applicable = False
                fs_reason = dte_metric.get("reason", "N_A_NO_STANDARD_DEBT_RULE_COMPANIES")
                
            if fs_applicable:
                dte_score = dte_metric.get("score")
