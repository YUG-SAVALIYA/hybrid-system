"""
FundamentalSectorAggregationService

Calculates raw fundamental aggregation for sectors.
"""
...
MIN_SECTOR_FUNDAMENTAL_COMPANIES = 5
...
def _aggregate_metric(values: List[float], applicable_count: int) -> Dict[str, Any]:
    ...
    if applicable_count == 0:
        ...
        coverage = (valid_count / applicable_weight) * 100.0 if 'applicable_weight' in locals() else (valid_count / applicable_count) * 100.0
...
class FundamentalSectorAggregationService:
    def __init__(self, discovery_session: Session):
        self._disc = discovery_session

    def aggregate_sectors(self, run_id: str) -> None:
        ...
        for c in companies:
            if not c.sector:
                continue
            sectors_map.setdefault(c.sector, []).append(c)
        ...
        group_by_name = {g.entity_name: g for g in existing_groups}
        ...
        for sector_name, mems in sectors_map.items():
            ...
            group_score = group_by_name.get(sector_name)
            if not group_score:
                import uuid
                group_score = GroupScore(
                    ...
                    parent_sector="",
                    parent_industry="",
                    horizon="1Y" # default or passed logic
                )
