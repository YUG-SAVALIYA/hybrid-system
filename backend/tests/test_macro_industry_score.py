"""Offline tests for deterministic industry macro scoring."""
import copy
import uuid
...
from services.macro.macro_industry_score import (
    CATEGORY_WEIGHTS,
    CONFIDENCE_MULTIPLIERS,
    IMPACT_NUMERIC_VALUES,
    MacroIndustryScoreService,
    ...
    W_STALE,
    W_UNAVAILABLE,
    calculate_industry_macro_score,
    _status_from_score,
)
...
def _score(category_impacts, overall="POSITIVE", relationship="MORE_POSITIVE"):
    return calculate_industry_macro_score(
        category_impacts,
        {"impact": overall, "relationship_to_parent_sector": relationship},
    )


def _make_group(session, run_id, sector="Tech", industry="Software", horizon="SHORT"):
    row = GroupScore(
        id=str(uuid.uuid4()),
        run_id=run_id,
...
def _make_impact(
    ...
    row = MacroEntityImpact(
        ...
        source_parent_impact_id="sector-impact",
        entity_type="INDUSTRY",
        entity_name=industry,
        parent_sector=sector,
        parent_industry="",
        ...
        overall_impact={
            "impact": overall,
            "confidence": "MEDIUM",
            "reason": "Overall industry impact.",
...
def _group(session, run_id, sector="Tech", industry="Software", horizon="SHORT"):
    return (
...
def test_partial_weight_normalization():
    impacts = _category_impacts("N_A", "LOW")
...
def test_low_coverage_preserves_score(disc_session):
    ...
    MacroIndustryScoreService(disc_session).calculate_industry_scores(run_id)
    group = _group(disc_session, run_id)

    assert group.macro_score == 100.0
    assert group.calculation_details["macro"]["industry_score"]["eligible_for_selection"] is False
...
def test_parent_relationship_does_not_alter_score():
    ...
    assert details_a["relationship_to_parent_sector"] == "MORE_POSITIVE"
    assert details_b["relationship_to_parent_sector"] == "MORE_NEGATIVE"
...
def test_one_impact_updates_all_matching_horizons(disc_session):
    ...
    metadata = MacroIndustryScoreService(disc_session).calculate_industry_scores(run_id)
...
def test_same_industry_name_under_different_sectors_isolated(disc_session):
    run_id = f"run_{uuid.uuid4().hex[:6]}"
    _make_group(disc_session, run_id, sector="Tech", industry="Software")
    _make_group(disc_session, run_id, sector="Auto", industry="Software")
    _make_impact(disc_session, run_id, sector="Tech", industry="Software", category_impacts=_category_impacts("POSITIVE", "HIGH"))
    _make_impact(disc_session, run_id, sector="Auto", industry="Software", category_impacts=_category_impacts("NEGATIVE", "HIGH"), overall="NEGATIVE")

    MacroIndustryScoreService(disc_session).calculate_industry_scores(run_id)

    assert _group(disc_session, run_id, "Tech", "Software").macro_score == 100.0
    assert _group(disc_session, run_id, "Auto", "Software").macro_score == 0.0
...
def test_stale_score_cleanup(disc_session):
    ...
    group.calculation_details = {"macro": {"industry_score": {"score": 75.0, "status": "POSITIVE"}}}
    disc_session.commit()
    ...
    assert group.calculation_details["macro"]["industry_score"]["reason"] == W_STALE
...
def test_cleanup_affects_only_supplied_run(disc_session):
    ...
    for row in (group, other):
        ...
        row.calculation_details = {"macro": {"industry_score": {"score": 75.0, "status": "POSITIVE"}}}
    disc_session.commit()
...
def test_idempotent_update(disc_session):
    ...
    _make_group(disc_session, run_id)
    _make_impact(disc_session, run_id)
    service = MacroIndustryScoreService(disc_session)

    first = service.calculate_industry_scores(run_id)
    group = _group(disc_session, run_id)
    first_calc = copy.deepcopy(group.calculation_details)
    first_warnings = list(group.warnings)
    second = service.calculate_industry_scores(run_id)
    disc_session.expire(group)
...
def test_no_llm_or_parallel_call(disc_session):
    ...
    service.calculate_industry_scores(run_id)
