"""
Tests for FundamentalIndustryTransitionScoreService.
"""
import uuid
import pytest
...
def test_fundamental_industry_transition_score_service(disc_session):
    run_id = "run_ind_trans"
    
    # 1. Net-profit fallback weighted score, 3. STANDARD_GROWTH numeric, 11. Share calculations
    _create_group(disc_session, run_id, "IND_MIXED", 
        np_counts={
    ...
    # 4. No fallback net-profit -> null score
    _create_group(disc_session, run_id, "IND_NUMERIC_ONLY", 
        np_counts={"STANDARD_GROWTH": 10},
        b_counts={},
        cc_counts={}
    )
    
    # 2. Every net-profit mapping
    _create_group(disc_session, run_id, "IND_ALL_NP", 
        np_counts={
            "LOSS_TO_PROFIT": 1, # 90
            "LOSS_NARROWED": 1, # 65
            "LOSS_UNCHANGED": 1, # 35
    ...
    _create_group(disc_session, run_id, "IND_BORROW", 
        np_counts={},
        b_counts={
            "ZERO_TO_ZERO": 2, # 90
    ...
    _create_group(disc_session, run_id, "IND_FINANCIAL", 
        np_counts={},
    ...
    _create_group(disc_session, run_id, "IND_ALL_CC", 
        np_counts={},
        b_counts={},
        cc_counts={
    ...
    _create_group(disc_session, run_id, "IND_EMPTY", {}, {}, {})
    
    disc_session.commit()
    
    svc = FundamentalIndustryTransitionScoreService(disc_session)
    ...
    # 1, 3, 11
    t_mixed = get_t("IND_MIXED")["net_profit"]
    ...
    # 12
    t_emp = get_t("IND_EMPTY")["net_profit"]
    assert t_emp["valid_status_count"] == 0
    assert t_emp["fallback_score"] is None
    assert t_emp["numeric_share_pct"] is None
    assert t_emp["fallback_share_pct"] is None
    
    # 13, 14
    g = disc_session.query(GroupScore).filter_by(entity_name="IND_MIXED").first()
    ...
    # 15
    svc.calculate_transition_scores(run_id)
    t_mixed2 = get_t("IND_MIXED")["net_profit"]
